async function renderCareersView(container) {
  container.innerHTML = `<section class="careers-catalog-card"><p>Cargando carreras...</p></section>`;
      ...
      <section class="careers-catalog-card">
        ...
        <ul class="careers-list">
          ${careers
            .map(
              (career) => `
                <li class="career-item career-item--clickable" data-id="${career.id}">
  ...
  async function renderDetail(careerId) {
    container.innerHTML = `<section class="careers-catalog-card"><p>Cargando...</p></section>`;

    try {
      const career = await apiGet(`/careers/${careerId}`);
      container.innerHTML = `
        <section class="careers-catalog-card">
          <button type="button" class="back-link" id="back-to-list">&larr; Volver al catálogo</button>
          <h1>${career.name}</h1>
